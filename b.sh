...
--- a/Package.swift
+++ b/Package.swift
@@ -40,8 +40,12 @@ if buildDynamicLibrary {
...
for module in `cd Sources; echo {Swift,_}*; cd VersionMarkerModules; echo SwiftSyntax600 SwiftSyntax601`; do
...
PACKAGE
    ...
    cd $DEST/$MODULE.xcframework && for VARIANT in *; do if [ $VARIANT != "Info.plist" ]; then
        ...
        cp -rf $SOURCE/build.*/Build/Products/$DIR/SwiftSyntax600.swiftmodule $VARIANT/$MODULE.swiftmodule ||
        cp -rf $SOURCE/build.*/Build/Products/$DIR/SwiftSyntax601.swiftmodule $VARIANT/$MODULE.swiftmodule
    fi done

    cd $DEST && rm -f $MODULE.xcframework.zip $MODULE.xcframework/*/*/*.swiftmodule &&
    codesign -f --timestamp -s "Apple Development: dkobus@motionvfx.com (482DSGB3Y5)" $MODULE.xcframework &&
